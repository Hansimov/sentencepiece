...
bool TrainerInterface::is_exceed_max_length(const string_util::UnicodeText &piece) const
{
  return str_utils::monospace_size(piece) > trainer_spec_.max_sentencepiece_length();
}
...
bool TrainerInterface::IsValidSentencePiece(
  ...
  if (str_utils::is_beg_or_end_with_one_char(sentencepiece))
    return false;
  if (str_utils::is_pure_digits(sentencepiece)) return false;
  if (str_utils::is_malformed(sentencepiece))
    return false;
